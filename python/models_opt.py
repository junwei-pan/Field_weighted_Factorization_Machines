    ...
    def __init__(self, layer_sizes=None, layer_acts=None, layer_keeps=None, layer_l2=None, kernel_l2=None,
        ...
        with self.graph.as_default():
            ...
            with tf.device(gpu_device):
                ...
                print 'l_trans.get_shape()', l_trans.get_shape()
                l_transTran = tf.transpose(l_trans, [ 0, 2, 1])
                print 'l_transTran.get_shape()', l_transTran.get_shape()
                l_left = tf.gather(l_trans, index_left, axis = 1)
                print 'l_left.get_shape()', l_left.get_shape()
                l_right = tf.gather(l_trans, index_right, axis =1)
                print 'l_right.get_shape()', l_right.get_shape()
                p_full = tf.multiply(l_left, l_right)
                print 'p_full.get_shape()', p_full.get_shape()
                ...
                ## Now for full Interaction 3 Layer
                p3_full = tf.matmul(p_full, l_transTran)
                print 'p3_full.get_shape()', p3_full.get_shape()

                p3_full = tf.multiply(p3_full, w_3)
                print 'p3_full.get_shape()', p3_full.get_shape()

                p3 = tf.reduce_sum(p3_full, 2)
                print 'p3.get_shape()', p3.get_shape()

                p3 = tf.reduce_sum(p3, 1, keep_dims = True)
                print 'p3.get_shape()', p3.get_shape()
                print 'b1.get_shape()', b1.get_shape()       
